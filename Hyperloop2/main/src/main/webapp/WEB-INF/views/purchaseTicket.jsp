    ...
    <main>
        <div class="container">
            ...
            <form action="/purchaseConfirmation" method="post">
                <h5>Your start fromStationId is ${scheduleEntry.fromStation.name}</h5>
                <div class="input-field">
                    <select name="arrStation">
                        ...
                        <c:forEach items="${stationList}" var="fromStationId">
                            <option
                                    <c:if test="${selectedStationID==fromStationId.fromStationId}">selected</c:if>
                                    value="${fromStationId.fromStationId}">${fromStationId.name}</option>
                        </c:forEach>
                        <c:if test="${selectedStationID == null}">
                            <option value="" selected disabled>Choose destination fromStationId</option>
                        </c:if>
                    </select>
                    <label>Destination fromStationId</label>
                </div>
                ...
                <input name="depStation" type="text" value="${scheduleEntry.fromStation.id}" hidden>
