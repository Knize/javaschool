...
public class PathFinderService {
    ...
    public void executeMax(StationEntity source) {
        settledNodes = new HashSet<>();
        unSettledNodes = new HashSet<>();
        distance = new HashMap<>();
        predecessors = new HashMap<>();
        ...
        unSettledNodes.add(source);
        while (unSettledNodes.size() > 0) {
            StationEntity node = getMaximum(unSettledNodes);
            settledNodes.add(node);
            unSettledNodes.remove(node);
            findMinimalDistances(node);
        }
    ...
    private StationEntity getMaximum(Set<StationEntity> vertexes) {
        StationEntity maximum = null;
        for (StationEntity vertex : vertexes) {
            if (maximum == null) {
                maximum = vertex;
            } else {
                if (getShortestDistance(vertex) > getShortestDistance(maximum)) {
                    maximum = vertex;
                }
            }
        }
